...
describe("calc", () => {
  describe("Multi-Gen", () => {
    ...
    tests("Comet Punch", ({ gen, calculate, Pokemon, Move }) => {
      expect(
        calculate(Pokemon("Snorlax"), Pokemon("Vulpix"), Move("Comet Punch"))
      ).toMatch(gen, {
        1: {
          range: [108, 129],
          desc: "Snorlax Comet Punch (3 hits) vs. Vulpix",
          result: "(38.7 - 46.2%) -- approx. 3HKO",
        },
        3: {
          range: [132, 156],
          desc: "0 Atk Snorlax Comet Punch (3 hits) vs. 0 HP / 0 Def Vulpix",
          result: "(60.8 - 71.8%) -- approx. 2HKO",
        },
    ...
    inGens(1, 9, ({ gen, calculate, Pokemon, Move }) => {
      test(`Immunity (gen ${gen})`, () => {
        expect(
          calculate(Pokemon("Snorlax"), Pokemon("Gengar"), Move("Hyper Beam"))
            .damage
        ).toBe(0);
      });
    });
    ...
    inGens(1, 9, ({ gen, calculate, Pokemon, Move, Field }) => {
      test(`Protect (gen ${gen})`, () => {
        ...
        expect(
          calculate(snorlax, chansey, Move("Hyper Beam"), field).damage
        ).toBe(0);
      });
    });
    ...
    inGens(3, 9, ({ gen, calculate, Pokemon, Move, Field }) => {
      test(`Weather Ball should change type depending on the weather (gen ${gen})`, () => {
        const weathers = [
          {
            ...
            damage: {
              adv: {
                range: [346, 408],
                desc: "(149.7 - 176.6%) -- guaranteed OHKO",
              },
              dpp: {
                range: [342, 404],
                desc: "(148 - 174.8%) -- guaranteed OHKO",
              },
              modern: {
                range: [344, 408],
                desc: "(148.9 - 176.6%) -- guaranteed OHKO",
              },
            },
          },
          {
            ...
            damage: {
              adv: {
                range: [86, 102],
                desc: "(37.2 - 44.1%) -- guaranteed 3HKO",
              },
              dpp: {
                range: [85, 101],
                desc: "(36.7 - 43.7%) -- guaranteed 3HKO",
              },
              modern: {
                range: [86, 102],
                desc: "(37.2 - 44.1%) -- guaranteed 3HKO",
        ...
        for (const { weather, type, damage } of weathers) {
          const dmg =
            gen === 3 ? damage.adv : gen === 4 ? damage.dpp : damage.modern;
          const [atk, def] =
            gen === 3 && type === "Rock" ? ["Atk", "Def"] : ["SpA", "SpD"];
    ...
    inGens(6, 9, ({ gen, calculate, Pokemon, Move }) => {
      test(`Flying Press (gen ${gen})`, () => {
        ...
        const ringTargetSpiritomb = Pokemon("Spiritomb", {
          item: "Ring Target",
        });
    ...
    inGens(4, 9, ({ gen, calculate, Pokemon, Move }) => {
      ...
      test(`Iron Ball negates levitate (gen ${gen})`, () => {
        const result = calculate(
          Pokemon("Poliwrath"),
          Pokemon("Mismagius", { item: "Iron Ball" }),
          Move("Mud Shot")
        );
    ...
    inGens(5, 9, ({ gen, calculate, Pokemon, Move, Field }) => {
      ...
      const dragonite1 = Pokemon("Dragonite", {
        ability: "Multiscale",
        curHP: 69,
      });
      const dragonite2 = Pokemon("Dragonite", {
        ability: "Energy Shield",
        item: "Heavy-Duty Boots",
      });
      if (gen > 7) {
        test(`Multiscale and Energy Shield halves damage even if there are hazzards if holding Heavy-Duty Boots (gen ${gen})`, () => {
          ...
          expect(result.desc()).toBe(
            "0 SpA Abomasnow Blizzard vs. 0 HP / 0 SpD Energy Shield Dragonite: 222-264 (68.7 - 81.7%) -- guaranteed 2HKO"
          );
        });
      }
      test(`Multiscale and Energy Shield should not halve damage if less than 100% HP (gen ${gen})`, () => {
        const result = calculate(
          Pokemon("Abomasnow"),
          dragonite1,
          Move("Ice Shard")
      ...
      test(`Multiscale and Energy Shield Should halve damage taken (gen ${gen})`, () => {
        const result = calculate(
          Pokemon("Abomasnow"),
          dragonite,
          Move("Ice Shard")
      ...
      describe("Weight", function () {
        describe("Heavy Metal", () => {
          ...
          it("should double the weight of a Pokemon", () =>
            expect(testBP("Gluttony")).toBe(80));
          it("should be negated by Mold Breaker", () =>
            expect(testBP("Mold Breaker")).toBe(60));
        });
        describe("Light Metal", () => {
          ...
          it("should halve the weight of a Pokemon", () =>
            expect(testBP("Gluttony")).toBe(100));
          it("should be negated by Mold Breaker", () =>
            expect(testBP("Mold Breaker")).toBe(120));
        });
        describe("Float Stone", () => {
          ...
          it("should halve the weight of a Pokemon", () =>
            expect(testBP()).toBe(100));
          it("should stack with Light Metal", () =>
            expect(testBP("Light Metal")).toBe(80));
        });
      });
    });
      ...
      test(`Expertise (gen ${gen})`, () => {
        ...
        if (gen === 6) {
          expect(result.damage).toEqual([
            [
              153, 154, 156, 157, 159, 162, 163, 165, 166, 168, 171, 172, 174,
              175, 177, 180,
            ],
            [76, 76, 78, 78, 79, 81, 81, 82, 82, 84, 85, 85, 87, 87, 88, 90],
          ]);
          expect(result.desc()).toBe(
            "152 Atk Expertise Kangaskhan-Mega Frustration vs. 252 HP / 152+ Def Amoonguss: 229-270 (53 - 62.5%) -- approx. 2HKO"
          );
        } else {
          expect(result.damage).toEqual([
            [
          ...
          expect(result.desc()).toBe(
            "152 Atk Expertise Kangaskhan-Mega Frustration vs. 252 HP / 152+ Def Amoonguss: 190-225 (43.9 - 52%) -- approx. 6.6% chance to 2HKO"
          );
        }
        ...
        expect(result.desc()).toBe(
          "Lvl 88 Expertise Kangaskhan-Mega Seismic Toss vs. 0 HP Amoonguss: 176-176 (47.6 - 47.6%) -- guaranteed 3HKO"
        );
        ...
        if (gen === 6) {
          expect(result.desc()).toBe(
            "252 Atk Expertise Kangaskhan-Mega Power-Up Punch vs. Lvl 72 0 HP / 0 Def Aggron: 248-296 (120.9 - 144.3%) -- guaranteed OHKO"
          );
        } else {
          expect(result.desc()).toBe(
            "252 Atk Expertise Kangaskhan-Mega Power-Up Punch vs. Lvl 72 0 HP / 0 Def Aggron: 196-236 (95.6 - 115.1%) -- 78.9% chance to OHKO"
          );
        }
        ...
        expect(result.damage).toEqual([
          [
            188, 190, 192, 194, 196, 198, 202, 204, 206, 208, 210, 212, 214,
            216, 218, 222,
          ],
          [
            92, 96, 96, 96, 96, 100, 100, 100, 104, 104, 104, 104, 108, 108,
            108, 112,
          ],
        ]);
        expect(result.desc()).toBe(
          "252 Atk Expertise Kangaskhan-Mega Crunch vs. 0 HP / 0 Def Energy Shield Lunala: 280-334 (67.4 - 80.4%) -- approx. 2HKO"
        );
      });
    });
    ...
    inGens(6, 9, ({ gen, calculate, Pokemon, Move }) => {
      test("Knock Off vs. Klutz", () => {
        ...
        const audino = Pokemon("Audino", {
          ability: "Klutz",
          item: "Leftovers",
        });
        const audinoMega = Pokemon("Audino", {
          ability: "Klutz",
          item: "Audinite",
        });
  ...
  describe("Gen 7", () => {
    inGen(7, ({ calculate, Pokemon, Move, Field }) => {
      ...
      test("Wring Out", () => {
        const smeargle = Pokemon("Smeargle", {
          level: 50,
          ability: "Technician",
        });
        const blissey = Pokemon("Blissey", {
          level: 50,
          evs: { hp: 252 },
          curHP: 184,
        });
      ...
      test("32-bit Overflow", () => {
        ...
        expect(calculate(kyogre, wynaut, waterSpout, field).range()).toEqual([
          55, 66,
        ]);

        kyogre = Pokemon("Kyogre", {
          ...kyogre,
          overrides: { types: ["Normal"] },
        });
        expect(calculate(kyogre, wynaut, waterSpout, field).range()).toEqual([
          37, 44,
        ]);
  ...
  describe("Gen 9", () => {
    inGen(9, ({ calculate, Pokemon, Move, Field }) => {
      test("Supreme Overlord", () => {
        const kingambit = Pokemon("Kingambit", {
          level: 100,
      ...
      test("Electro Drift/Collision Course boost on Super Effective hits", () => {
        ...
        const calc = (move = Move("Electro Drift")) =>
          calculate(attacker, defender, move).range();
      ...
      function testQP(
        ...
        test(`${ability} should take into account boosted stats by default`, () => {
          const attacker = Pokemon("Iron Leaves", {
            ability,
            boostedStat: "auto",
            boosts: { spa: 6 },
          });
          // highest stat = defense
          const defender = Pokemon("Iron Treads", {
            ability,
            boostedStat: "auto",
            boosts: { spd: 6 },
          });
      ...
      function testQPOverride(
        ...
        test(`${ability} should be able to be overridden with boostedStat`, () => {
          ...
          // highest stat = defense
          const defender = Pokemon("Walking Wake", {
            ability,
            boostedStat: "def",
            boosts: { spd: 6 },
          });
      ...
      describe("Tera Stellar", () => {
        ...
        test("should only be displayed on defender for Stellar attacks", () => {
          expect(
            calculate(control, terastal, Move("Tera Blast")).rawDesc
              .defenderTera
          ).toBeUndefined();
          expect(
            calculate(terastal, terastal, Move("Tera Blast")).rawDesc
              .defenderTera
          ).toBeDefined();
          ...
          expect(
            calculate(
              terastal,
              terastal,
              Move("Tera Blast", { isStellarFirstUse: true })
        ...
        test("should not be displayed for non-boosted attacks", () =>
          expect(
            calculate(
              terastal,
              control,
              Move("Judgment", { isStellarFirstUse: false })
            ).rawDesc.attackerTera
          ).toBeUndefined());
        test("should distinguish between first use for Tera Blast", () => {
          // I don't exactly care what the difference is
          const result = [true, false].map(
            (isStellarFirstUse, ..._) =>
              calculate(
                terastal,
                control,
                Move("Tera Blast", { isStellarFirstUse })
              ).rawDesc.isStellarFirstUse
          );
    ...
    describe("Some moves should break screens before doing damage", () => {
      inGens(3, 9, ({ calculate, Pokemon, Move, Field }) => {
        test("Brick Break should break screens", () => {
          ...
          const otherMove = Move("Vital Throw", {
            overrides: { basePower: 75 },
          });

          const field = Field({ defenderSide: { isReflect: true } });

          const brickBreakResult = calculate(
            pokemon,
            pokemon,
            brickBreak,
            field
          ...
          expect(brickBreakResult.range()[0]).toBeGreaterThan(
            otherMoveResult.range()[0]
          );
          expect(brickBreakResult.range()[1]).toBeGreaterThan(
      ...
      inGens(7, 9, ({ calculate, Pokemon, Move, Field }) => {
        test("Psychic Fangs should break screens", () => {
          ...
          const otherMove = Move("Zen Headbutt", {
            overrides: { basePower: 75 },
          });

          const field = Field({ defenderSide: { isReflect: true } });

          const psychicFangsResult = calculate(
            pokemon,
            pokemon,
            psychicFangs,
            field
          ...
          expect(psychicFangsResult.range()[0]).toBeGreaterThan(
            otherMoveResult.range()[0]
          );
          expect(psychicFangsResult.range()[1]).toBeGreaterThan(
            otherMoveResult.range()[1]
          );
        });
      });
      inGen(9, ({ calculate, Pokemon, Move, Field }) => {
        test("Raging Bull should break screens", () => {
          ...
          const ragingBullResult = calculate(
            pokemon,
            pokemon,
            ragingBull,
            field
          ...
          expect(ragingBullResult.range()[0]).toBeGreaterThan(
            otherMoveResult.range()[0]
          );
          expect(ragingBullResult.range()[1]).toBeGreaterThan(
            otherMoveResult.range()[1]
          );
        });
      });
