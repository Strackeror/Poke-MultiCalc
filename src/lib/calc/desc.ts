...
export function display(
  ...
  const min =
    typeof minDamage === "number" ? minDamage : minDamage[0] + minDamage[1];
  const max =
    typeof maxDamage === "number" ? maxDamage : maxDamage[0] + maxDamage[1];
  ...
  const koChanceText = getKOChance(
    gen,
    attacker,
    defender,
    move,
...
export function displayMove(
  ...
  const recoveryText = getRecovery(
    gen,
    attacker,
    defender,
    move,
...
export function getRecoil(
  ...
  if (move.recoil) {
    ...
    if (!attacker.hasAbility("Headstrong")) {
    ...
    switch (gen.num) {
      ...
      case 4:
        if (defender.hasType("Ghost")) {
          if (gen.num === 4) {
            const gen4CrashDamage = Math.floor(
              ((defender.maxHP() * 0.5) / attacker.maxHP()) * 100
            );
            recoil =
              notation === "%"
                ? gen4CrashDamage
...
export function getKOChance(
  ...
  const hazardsText =
    hazards.texts.length > 0 ? " after " + serializeText(hazards.texts) : "";
  const afterText =
    hazards.texts.length > 0 || eot.texts.length > 0
      ? " after " + serializeText(hazards.texts.concat(eot.texts))
      : "";
  const afterTextNoHazards =
    eot.texts.length > 0 ? " after " + serializeText(eot.texts) : "";
  ...
  function KOChance(
    ...
    const KOTurnText =
      n === 1 ? "OHKO" : multipleTurns ? `KO in ${n} turns` : `${n}HKO`;
    let text = qualifier;
    ...
    if (chanceWithoutEot === undefined || chanceWithEot === undefined) {
      ...
      if (chanceWithEot === 1) {
        text +=
          `${roundChance(
            chanceWithoutEot
          )}% chance to ${KOTurnText}${hazardsText} ` +
      ...
      if (chanceWithEot === 1) {
        ...
        text += `${roundChance(
          chanceWithEot
        )}% chance to ${KOTurnText}${afterText}`;
      }
    }
  ...
  if ((move.timesUsed === 1 && move.timesUsedWithMetronome === 1) || move.isZ) {
    ...
    // Expertise's combined first + second hit only is accurate for chance to OHKO, for
...
function combine(damage: Damage) {
  ...
  if (damage.length > 2) {
    if (damage[0] > damage[damage.length - 1])
      damage = damage.slice().sort() as number[];
    return damage as number[];
  }
  // Fixed Expertise Damage
...
function getHazards(gen: Generation, defender: Pokemon, defenderSide: Side) {
  ...
  if (
    defenderSide.steelsurge &&
    !defender.hasAbility("Magic Guard", "Mountaineer")
  ) {
...
function getEndOfTurn(
  ...
  const loseItem =
    move.named("Knock Off") && !defender.hasAbility("Sticky Hold");
  ...
  if (defender.hasStatus("psn")) {
    ...
    if (defender.hasAbility("Heatproof")) {
      ...
      damage -= Math.floor(
        defender.maxHP() / (gen.num === 1 || gen.num > 6 ? 16 : 8)
      );
  ...
  if (defender.isSaltCure && !defender.hasAbility("Magic Guard")) {
    const isWaterOrSteel =
      defender.hasType("Water", "Steel") ||
...
function squashMultihit(
  ...
  if (d.length === 1) {
    ...
    if (hits > 1) {
      error(err, `Unexpected # of hits for Expertise: ${hits}`);
    }
    // FIXME: Come up with a better Expertise approximation
    const r: number[] = [];
    for (let i = 0; i < 16; i++) {
      let val = 0;
      for (let j = 0; j < 16; j++) {
...
function buildDescription(
  ...
  const [attackerLevel, defenderLevel] = getDescriptionLevels(
    attacker,
    defender
  );
  let output = "";
  ...
  if (description.alliesFainted) {
    output +=
      Math.min(5, description.alliesFainted) +
