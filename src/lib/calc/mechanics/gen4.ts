...
export function calculateDPP(
  ...
  if (secondDefenderType && firstDefenderType !== secondDefenderType) {
    const firstTypePrecedence =
      typeEffectivenessPrecedenceRules.indexOf(firstDefenderType);
    const secondTypePrecedence =
      typeEffectivenessPrecedenceRules.indexOf(secondDefenderType);
  ...
  if (
    ...
    (move.hasType("Water") &&
      defender.hasAbility("Dry Skin", "Water Absorb")) ||
  ...
  let basePower = calculateBasePowerDPP(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackDPP(
    gen,
    attacker,
    defender,
    move,
  ...
  const defense = calculateDefenseDPP(
    gen,
    attacker,
    defender,
    move,
  ...
  baseDamage = calculateFinalModsDPP(
    baseDamage,
    attacker,
    move,
    field,
  ...
  if ((move.dropsStats && move.timesUsed! > 1) || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      let newBasePower = calculateBasePowerDPP(
        gen,
        attacker,
        defender,
        move,
      ...
      newBasePower = calculateBPModsDPP(
        attacker,
        defender,
        move,
        field,
      ...
      const newAtk = calculateAttackDPP(
        gen,
        attacker,
        defender,
        move,
      ...
      let baseDamage = Math.floor(
        Math.floor(
          (Math.floor((2 * attacker.level) / 5 + 2) * newBasePower * newAtk) /
            50
        ) / defense
      );
      if (
      ...
      result.damage = result.damage.map((affectedAmount) => {
        ...
        newFinalDamage = Math.floor(
          (baseDamage * (85 + damageMultiplier)) / 100
        );
...
export function calculateBasePowerDPP(
  ...
  switch (move.name) {
    ...
    case "Water Spout":
      basePower = Math.max(
        1,
        Math.floor((basePower * attacker.curHP()) / attacker.maxHP())
      );
      desc.moveBP = basePower;
      break;
    case "Facade":
...
export function calculateBPModsDPP(
  ...
  if (
    ...
    move.hasType(getItemBoostType(attacker.item)) ||
...
export function calculateAttackDPP(
  ...
  desc.attackEVs = getStatDescriptionText(
    gen,
    attacker,
    attackStat,
    attacker.nature
  ...
  if (isPhysical && attacker.hasAbility("Pure Power", "Huge Power")) {
    ...
    (isPhysical &&
      (attacker.hasAbility("Hustle") ||
        (attacker.hasAbility("Guts") && attacker.status))) ||
...
function calculateFinalModsDPP(
  ...
  if (
    (field.hasWeather("Sun") && move.hasType("Fire")) ||
