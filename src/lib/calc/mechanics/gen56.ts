...
export function calculateBWXY(
  ...
  if (move.named("Weather Ball")) {
    ...
    if (gen.num === 5) {
      ...
      move.type = field.hasTerrain("Electric")
        ? "Electric"
  ...
  const attack = calculateAttackBWXY(
    gen,
    attacker,
    defender,
    move,
  ...
  const defense = calculateDefenseBWXY(
    gen,
    attacker,
    defender,
    move,
  ...
  if ((move.dropsStats && move.timesUsed! > 1) || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAtk = calculateAttackBWXY(
        gen,
        attacker,
        defender,
        move,
      ...
      const newDef = calculateDefenseBWXY(
        gen,
        attacker,
        defender,
        move,
...
export function calculateAtModsBWXY(
  ...
  if (
    ...
    (attacker.hasItem("Light Ball") &&
      attacker.name.startsWith("Pikachu") &&
      !move.isZ)
  ) {
    atMods.push(8192);
    desc.attackerItem = attacker.item;
...
function calculateFinalModsBWXY(
  ...
  if (
    ...
    finalMods.push(
      field.gameType !== "Singles" ? (gen.num > 5 ? 2732 : 2703) : 2048
    );
    desc.isReflect = true;
  } else if (
